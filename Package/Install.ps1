...
try {
    ...
    elseif ($existingIsREFramework) {
        ...
        Write-Host 'The managed fallback requires REFramework Plugin API 1.15. If the existing build rejects it, rerun Install.ps1 with -ReplaceExistingFramework; the original DLL will be backed up and restored on uninstall.' -ForegroundColor Yellow
    }
    ...
    Write-Host ''
    Write-Host 'DMC5 DualSense Layer was installed successfully.' -ForegroundColor Green
